...
Open with one short message that holds two things and nothing else: the permission-mode check below and the one sentence about restarts above. Then wait for their answer. Do not list what you have already looked at, do not recap this guide and do not preview the steps. Start Step 1 as soon as they have answered.
...
A browser saves a second download of the same file with "(1)" in its name, so the person may have more than one copy, and the plain name may be the older one. The person's sentence asks for the newest copy, so you will already have picked one to read. Once the permission mode is settled, confirm it: list the other files in the same folder whose names start with `claude-code-power-setup-guide`, and read the `Guide version` line at the top of each (you do not need to read more of them). Follow the copy with the highest version, and treat a copy with no `Guide version` line as older than any copy that has one. If two say the same, follow the newest. If the copy you started from is older, say so and follow the newer one from the top of this section. Keep this out of your opening message. In your next message, tell the person in one line which copy you are using ("You have two copies of the guide. I'm using the one that says version 3.2 and leaving the other alone."). Never rename, move or delete any copy. Write the full path of the copy you follow into `SETUP-PROGRESS.md` exactly as it is, `(1)` and all, and put quotes around it in every command.
...
5. **Record it.** Write `Overview shown: yes` in `SETUP-PROGRESS.md`. That file is created right after Step 1, so if it doesn't exist yet, remember the answer and write the line when you create it. If you skipped the picture, write `Overview shown: skipped` with a few words on why (for example "could not download it" or "they said no"), so it isn't tried again after a restart.
...
Don't assume. In your first message of this step, ask only the tools question: "What tools do you use day to day for your work? Notion, Xero, Stripe, HubSpot, Google Drive, spreadsheets, anything specific to your role." Write down what they tell you, this determines what you actually connect later, not a fixed list everyone gets regardless of relevance.
...
(a) **Nothing is ever written into a skill folder Claude didn't create itself.** If they already have a skill with the same name as one in the repo, theirs is never overwritten or changed. Only if they clearly say so is it moved to a backup folder first, and it is never deleted.
