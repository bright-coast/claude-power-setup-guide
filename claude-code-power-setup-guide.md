...
- **A shorter start.** Claude opens setup with the permission-mode check and one line about restarts, then carries on with the picture and Step 1.
...
Open with one short message that holds two things and nothing else: the permission-mode check below and the one sentence about restarts above. Then wait for their answer. Do not list what you have already looked at, do not recap this guide and do not preview the steps. Once they have answered, carry on with the copy check and the picture below, then Step 1, without asking anything else first. After a restart, repeat only the permission-mode check (a restart can bring back the starting mode) and leave out the sentence about restarts.
