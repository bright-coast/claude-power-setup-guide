...
3. If you have just downloaded and checked this exact file in this same run (the guide's Step 7 does this for Skill Updates itself), use that checked copy and skip the download. Otherwise download `skills/<id>/SKILL.md` to a temporary file. Check its fingerprint against the catalog's `sha256`, and its frontmatter `name` and `version`. Only one file is installed. Extra files are not supported yet.
