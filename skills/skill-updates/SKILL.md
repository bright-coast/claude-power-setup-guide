...
**What it writes.** Apart from its three small files (and the catalog file a check downloads into the computer's temporary folder), all of this happens only after a yes. The three small files are `.seen.json` (the new skills the person has already been told about), `.skipped.json` (the updates the person said no to) and `.lastcheck` (the date of the last successful check, so the quiet check runs at most once a week). Claude saves them without a separate question, so it can remember those answers and that date. Nothing is installed, updated or removed that way. When Skill Updates itself is installed, it writes `.seen.json` and `.lastcheck` (setup has just shown the person the skills and fetched the catalog, so the first Startup check is not due for a week). `.skipped.json` is made the first time the person says no to an update. If a file is missing (for example, Skill Updates was installed another way), a missing `.skipped.json` means an empty list, a missing `.lastcheck` means a check is due, and a missing `.seen.json` means the next check makes it as a baseline (see "The seen list").
...
- **Home folder.** Work out the person's home folder once, and write the full path out as literal text in every command (for example the path that `$HOME` gives). Never rely on `~` in PowerShell, because PowerShell does not expand `~` for programs it starts. macOS and Linux: `echo $HOME`. Windows PowerShell: `$env:USERPROFILE`. Git Bash: `cygpath -w "$HOME"` (see the next bullet). Below, `<skills>` means `<home>/.claude/skills` and `<backups>` means `<home>/.claude/skills-backup`. The backup folder is deliberately outside the skills folder, so nothing is ever loaded twice.
- **Which shell on Windows.** If the shell is Git Bash rather than PowerShell, use the Linux column of the commands below (the one-command forms work in Git Bash as they are), and put every path in quotes (user names can contain spaces). **Use one style of path throughout a single command, and that style is Windows-style paths** such as `C:\Users\yourname\.claude\skills`, the same style that `cygpath -w` prints for the temporary folder. Get the home folder as `cygpath -w "$HOME"`, and do not mix Windows-style paths with `$HOME`-based ones such as `/c/Users/yourname` in the same command. Make the temporary folder as `cygpath -w "$(mktemp -d)"` (see the notes under the table).
...
Skill Updates keeps the date of its last successful check in one small file, `<skills>/skill-updates/.lastcheck`. It holds a single line with a date, for example `2026-09-26` (year, month, day). Reading it with the file-reading tool needs no prompt in the usual case (see "Permission prompts" under "The basics"), so a check that is not due costs nothing.
...
4. **When one is due,** run "Check for updates". Once the catalog has passed steps 3 and 4 of "The catalog: get it and check it" (the format and the never-retarget check), write today's date to `.lastcheck`, and nothing else in it, with the editing tool. A check that is not due costs no prompts. A check that is due costs two: the download and this write. It costs one more only when `.seen.json` has to be written as well, because it is missing (the fetch plus two writes in all) or a new skill was just mentioned. If the check could not run, or the catalog was refused, do not write it, so the next session tries again.
...
6. **Only keep `.lastcheck` if this skill's own folder has an `.upstream.json`.** Otherwise do not write there (never write into a folder you did not create). In that case the quiet check runs once per session, because there is nowhere to keep a date, so it cannot be weekly.
...
3. **Compare.**
   ...
   - New skills: catalog skills that are `ready`, are not installed, and are not in the seen list (below). If the seen list does not exist yet, quiet mode announces none and makes the list as a baseline, and full mode lists the ready skills that are not installed once and then makes the baseline (see "The seen list").
...
5. **The seen list.** Keep `<skills>/skill-updates/.seen.json`: a JSON list of catalog ids the person has already been told about, for example `["ask-rob", "meeting-prep"]`. Installing Skill Updates writes it (see step 8 of "Install a skill"), holding the ids of the ready skills the person was just shown. **If it is missing anyway, for example because Skill Updates was installed another way, the check makes a baseline.** In quiet mode, write the file holding the ids of all skills that are `ready` in the catalog right now (installed or not), recording them as already seen, and say nothing about new skills on that run, because setup shows the full list. If Skill Updates was installed another way, the person can say "any new skills" at any time to see the full list. In full mode (the person asked for the check), do not stay silent: list the ready skills that are not installed once, then write the same baseline. Full mode also reports updates to installed skills. From then on, only mention new skills that are neither installed nor in this list, and add each one to the list right after you mention it, so a skill that becomes ready later is mentioned once. Updates never touch this file. If the person asks directly ("any new skills"), list every ready skill that is not installed, whether or not it is in the list, and add them all to it. Only keep this file if this skill's own folder has an `.upstream.json`. Otherwise do not write there, and just mention new skills without a list.
...
5. **Move aside if they chose (b), then write.** Only now, after the yes in step 4, and immediately before writing:
   ...
   - Write `.upstream.json` last, with the editing tool. It is one write. When the skill being installed is `skill-updates` itself, that same write already includes `guidePath` (see step 8), so it is never written twice:
...
8. **Only when the skill being installed is `skill-updates` itself:**
   - Include a `guidePath` field in the same `.upstream.json` write as step 5, so it is one write with `guidePath` in it and not a second write. It is the full path of the guide file the person is using, if you know it (during setup, it is the file you are following). If you do not know it, leave it out. The guide check will ask.
   ...
   - Once `.upstream.json` has been written and read back, write `.seen.json` with the editing tool, holding exactly the ids of the ready skills the person was just shown in this conversation, or `[]` if they were shown none. Then write `.lastcheck` holding today's date (setup has just fetched the catalog, so the first Startup check is not due for a week). That makes Skill Updates itself about six prompts in all.
   - Do not create `.skipped.json` now. It is made the first time the person says no to an update, and a missing file means an empty list. All three small files are only kept because this skill's own folder now has an `.upstream.json`. If `.seen.json` is missing later (for example, Skill Updates was installed another way), the first check makes a baseline instead (see "The seen list").
...
- Never write into a folder you did not create (this is about skill folders; the guide-folder writes are listed separately and each needs a yes). The only skill-folder exceptions are the three listed under "What this does", each with a yes first.
...
- Never run a downloaded script without showing it to the person first. A script the read-out named, and the person said yes to, may run without a second showing. A script not named, or new in an update, is shown in full first.
