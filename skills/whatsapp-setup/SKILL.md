...
- **The other kit files** (the scripts, templates and the kit's own skill) are covered only by the commit pin. The commit fixes what they contain, but this skill does not fingerprint them one by one.
- **The `wacli` tool and the voice note (speech) tool** are made by other people and have not been reviewed by Bright Coast AI.

Note for Rob (this does not change what you do): whenever the kit is updated, read the new version, then update the commit, both fingerprints and the file list in Step 3 here, and re-check the "early version" statements above and the expected downloads in Step 3 against it. The release check (`check-release`) does not watch the kit's repo and will not catch a change there, so this is a manual step. Until it is done, every install keeps using the version in this table.

## Step 0: Where do things stand?

Check these in order and stop at the first one that applies.

1. **The kit's own skill is in this folder.** Look for `.claude/skills/whatsapp/SKILL.md` in the folder Claude is working in. If it is there, WhatsApp is already set up here. Read that file and follow it for everything from now on, instead of the rest of this file. This skill is called `whatsapp-setup` and the kit's own skill is called `whatsapp`, so the two do not clash.
...
2. **What Claude reads leaves your computer.** Everything Claude reads is processed by Anthropic, the company that runs Claude. New photos, PDFs, documents and voice notes in the messages it reads are opened automatically, without asking, and that includes anything sensitive such as an ID or a bank statement.
...
7. **The `wacli` tool and the voice note tool are somebody else's.** The setup installs whichever release of `wacli` GitHub calls the latest on the day, and a fixed version of the speech tool `faster-whisper` from the Python package index. Bright Coast AI has not reviewed either one. The setup checks the `wacli` download against the checksum published with that release, which catches a damaged download but does not prove the release itself is safe.
...
- On the person's side: type a code into the phone (an 8 character code, entered in WhatsApp under Settings, Linked devices), quit and reopen Claude Code Desktop once, answer five short questions about how they write, click Allow on many commands, and near the end run a harmless test of the send check. Click plain Allow each time and never choose "always" or "do not ask again".
...
3. **Check the fingerprints.** Work out the SHA-256 of each downloaded file and compare all 64 characters with the table above.
   ...
   - **If either does not match, treat the kit as unreviewed.** Stop. Tell the person plainly that Bright Coast AI has not reviewed this version, and show them both fingerprints (expected and actual). Then show them what the file asks them to do: its "What this does" and "What you will be asked to do" sections and its "RULES FOR THE WHOLE SETUP", word for word. Go on only if they explicitly say yes after seeing that, and recommend they ask Rob first. If they say yes, carry on with the file you downloaded and no other, and show every script in full, not just a summary, and wait for a yes on each one before it runs (item 8).
...
6. **Check it against this skill.** The kit's setup is expected to download or contact exactly the following, and nothing else. Each one is shown to the person first: the kit says in one plain sentence what each command does before it runs, and item 8 has you show each script before it runs.
   - **The `wacli` tool.** The kit's install script asks GitHub which release of `wacli` (github.com/openclaw/wacli, made by other people) is the latest, downloads the file for this computer and that release's `checksums.txt`, and compares the file's SHA-256 with the value in `checksums.txt`. If they differ it deletes the download and stops. This catches a damaged or swapped download but does not prove the release is safe, because both files come from the same release page. On a Mac that already has Homebrew, the script instead installs `wacli` with Homebrew, which does its own checksum check, and then checks the code signature. It never installs Homebrew.
...
7. **Ask once more.** "This is what I am about to follow. Is that okay?" Do not go on until they say yes.
8. **Scripts.** The kit has small scripts (installing `wacli`, checking for pairing problems, setting up voice notes). On Windows the kit runs them with PowerShell's script safety check switched off for that one run (`-ExecutionPolicy Bypass`), which is why looking first matters. Before the first time each one runs, show the person its contents and say in one sentence what it does. Tell them they can read them any time in `~/WhatsAppAgent/kit/scripts`.
9. **Start it in the right folder.** The kit needs a session that starts inside the `WhatsAppAgent` folder, because Claude Code only loads a folder's safety rules and skills when a session starts there. Check the folder Claude is working in.
   ...
   - If not, tell the person: open Claude Code Desktop, start a new session, choose the `WhatsAppAgent` folder as the working folder, and paste this one line:
     ...
     `Start the WhatsApp Agent Kit setup. Read kit/SETUP-PROMPT.md and follow Part One, the prompt in the box under "The prompt", from Step 0. Before you run any script from kit/scripts for the first time, show me what it does.`
...
From here the kit's own instructions run the setup: they install `wacli`, check for known pairing problems, link the phone with a code, do one bounded first copy of messages, install the kit's files, and ask the person to quit and reopen Claude Code Desktop once. The kit tells the person how to carry on in the new session.
...
- The kit's own skill now takes over. It is called `whatsapp` and lives inside the `WhatsAppAgent` folder. It knows the commands, the digest format and the safety rules. If this file loads there, Step 0 hands over to it.
- Every summary should say how complete the data is. A newly linked device only gets recent history, and that is normal.
- If something goes wrong, Claude writes what happened to a file called `FEEDBACK.md` in the `WhatsAppAgent` folder and shows it. The person sends it to Rob themselves. Claude never sends it.
...
- Show the person what you are about to follow before you follow it (Step 3). Follow only the pinned copy that passed the fingerprint check, or a copy that failed it and that the person explicitly approved after you showed them what it asks.
...
- Never send a WhatsApp message, create a group or add anyone to a group unless the person has said to do that specific thing in this conversation. Never during setup.
