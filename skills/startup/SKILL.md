...
**What it checks online.** At most once a week it asks the Skill Updates skill to read GitHub's public catalog of the guide's skills, to see whether anything is newer. Nothing of yours is sent with that check. It stays quiet unless something is newer or the check could not run, and if it could not run it tries again next session. The very first check only notes which skills already exist, so it announces no new skills; after that, a skill that becomes ready later is mentioned once. The catalog is read again when you set up your startup or ask to add a skill.
